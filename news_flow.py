...
def llm_request(content):
    ...
    """
    messages = []
    messages.append({"role": "user", "content": content + "사건 흐름 중 뉴스 순서좀 년도에 맞게 총 3개로 나눠서 알려줘."})
    ...
    for key, value in news_dict.items():
        value['body'] = '\n'.join([line for line in value['body'].split('\n')[1:] if line.strip()])
    ...
    for key, value in news_dict.items():
        if value['body']:  # 내용이 있는 경우에만 포맷 만들기
            formatted_results.append({'role': 'user', 'content': f'{current_number}번: '})
    ...
    # 딕셔너리를 텍스트 파일에 저장
    with open("/Users/seojun/vscode/agentmaster/data/news_flow.txt", "w") as file:
        for result in formatted_results:
