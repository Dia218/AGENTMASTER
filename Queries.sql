...
/*해당 페이지에서 무작위로 뉴스 기사 5개를 골라 신문사 이름, 기사 제목, 분야를 출력한다. 기사의 제목을 클릭하면 해당 기사의 상세 페이지로 넘어간다. 이 때 해당 기사의 id를 속성으로 넘겨준다.*/
...
/*해당 페이지에서 등락률 기준 최상위 4개 종목의 종목명, 주가, 전일비, 등락률을 차례대로 출력한다.*/
...
/*해당 페이지에서 아이디와 비밀번호를 입력하고 로그인 버튼을 클릭 시 무조건 관리자 계정의 정보를 넘겨받고 관리자 계정의 id를 출력한다.*/
...
/*해당 페이지에서 아이디와 비밀번호, 이메일을 입력하고 회원 가입 버튼을 클릭하면 입력한 데이터를 db에 저장한다.*/
...
/*해당 페이지에서 클릭한 기사의 기사 제목, 스크랩 여부, 신문사 이름, 기자 이름을 출력한다.*/
->
...
/*해당 페이지에서 클릭한 기사의 요약문과 해당 기사의 원본 링크를 출력한다.*/
...
/*해당 페이지에서 클릭한 기사에 사건의 흐름이 존재할 경우, 해당 사건의 정보를 출력하고 그 옆에 해당 사건의 흐름에 속하는 뉴스 기사의 제목과 요약을 순서대로 출력한다.
->
사건 흐름과 관련해서는 현재 인공지능 파트와 논의중에 있어 현재는 쿼리문을 도출할 수 없습니다.*/


/*해당 페이지에서 클릭한 기사에 사건의 흐름이 존재하지 않는 경우, "해당 사건의 정보가 존재하지 않는다"고 출력한다.
->
사건 흐름과 관련해서는 현재 인공지능 파트와 논의중에 있어 현재는 쿼리문을 도출할 수 없습니다.*/


/*해당 페이지에서 클릭한 기사와 연관된 뉴스 기사 제목을 5~6개 출력한다. 이때 제목을 클릭할 경우 해당 기사의 상세 페이지로 넘어간다.*/
SELECT
...
/*해당 페이지에서 키워드를 입력해 검색하면 해당 키워드를 포함하는 뉴스 기사의 목록을 출력한다.
...
/*오늘의 뉴스 부분에서 무작위로 신문사 이름과 기사 요약(한줄 정도)를 띄운다.*/
...
/*검색창 부분에서 사용자가 검색한 키워드를 디비에 저장한다.
->
검색한 키워드는 데이터베이스에 저장되지 않습니다. 따로 필요하다면 테이블을 추가해야 합니다.*/


/*상한가, 하한가, 상승, 하락, 보합 버튼을 클릭했을 때 그에 맞는 순위 5위 정도의 종목을 현재가, 전일비, 등락률, 거래량, 매수호가, 매도호가, 매수총잔량, 매도총잔량의 정보를 띄운다.*/
...
JOIN (
	...
	FROM(
		SELECT
			stock_id, COUNT(stock_id) AS cid
		FROM "AGENTMASTER"."Stock_info"
...
/*디비에 저장되었던 검색한 키워드에 관한 기사들을 한줄 정도 띄운다.*/
...
/*디비에 저장된 종목 데이터(전일, 고가, 거래량, 시가, 저가, 거래대금)을 테이블 형식으로 출력한다.
->
